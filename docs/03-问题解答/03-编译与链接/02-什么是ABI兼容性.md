...
[extern-C与动态库导出](../01-基础概念/09-extern-C与动态库导出.md) 讲过 `extern "C"` 的核心作用：**禁用名称改编**。
...
[什么是不透明指针](../01-基础概念/06-什么是不透明指针.md) 讲过不透明指针的原理。Pimpl（Pointer to Implementation）就是不透明指针在 C++ 中的经典应用。
