...
> **前置阅读**：如果你还没有安装GCC/G++，请先阅读 [FAQ-138：开发环境配置详解](../../03-问题解答/01-基础概念/33-开发环境配置.md) 完成编译器安装。本文档假设你已经安装好了GCC/G++，需要深入学习编译参数和使用技巧。

> **相关教程**：编译器的基础概念见 [编译与链接](../../01-C语言/17-编译与链接.md)，编译器入门用法见 [GCC/G++编译器详解](../../01-C语言/20-GCC与G++编译器.md)。
