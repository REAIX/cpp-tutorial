> **前置知识**：高级内存管理见 [高级内存管理](./01-高级内存管理.md)，内存模型见 [原子操作与异步编程](../../02-CPP/29-原子操作与异步编程.md)。
...
| 读者类型 | 建议阅读范围 | 跳过内容 |
|---------|------------|---------|
| **初学者** | 🟢 第1章（CPU缓存层次结构）、🟢 第2章（缓存行与对齐） | 第3章及以后 |
| **中级开发者** | 🟢 第1-2章、🟡 第3章（数据布局优化）、🟡 第4章（缓存友好的数据结构） | 🔴 第6-7章 |
| **高级开发者/专家** | 全文阅读，重点关注 🔴 第6章（缓存一致性协议MESI）、🔴 第7章（false sharing深度分析与消除） | — |
